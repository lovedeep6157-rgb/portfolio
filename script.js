...
// ── ACADEMICS BAR ANIMATE ──
const academicBars = document.querySelectorAll('.academic-bar');
const academicObs = new IntersectionObserver((entries) => {
  entries.forEach(e => {
    if (e.isIntersecting) {
      const bar = e.target;
      bar.style.width = bar.dataset.width + '%';
      academicObs.unobserve(bar);
    }
  });
}, { threshold: 0.3 });

academicBars.forEach(b => academicObs.observe(b));
